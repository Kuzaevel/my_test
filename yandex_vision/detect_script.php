...
//$out = request_analyze_custom('resources/test_vision.png');
//echo "<pre>";
...
$analyze = request_analyze_custom($data->img_path);
$blocks = $analyze[0]->results[0]->textDetection->pages[0]->blocks;

$out = [];
foreach ($blocks as $block) {
    foreach ($block->lines as $line){
        foreach ($line->words as $word){
            array_push($out, $word->text . " - confidence: " . $word->confidence);
        }
    }
}
